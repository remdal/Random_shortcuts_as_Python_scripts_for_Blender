...
total_frames = int(duration_seconds * 120)
def get_precise_curve_length(curve_obj):
    ...
    return total_length
#curve_length = get_precise_curve_length(curve)
#plane_length = bone_length * num_bones
bpy.ops.curve.primitive_bezier_circle_add(radius=scale_factor / (2 * math.pi), location=(0, 0, 0))
...
'''
WHEELS_ORIGINAL = {
    "Wheel_Haut": {"vertices": 13, "pos": (0, 0.3, 0), "radius": 0.1751},
    "Wheel_Gauche": {"vertices": 10, "pos": (-0.3, 0, 0), "radius": 0.1273},
    "Wheel_Droite": {"vertices": 10, "pos": (0.6, 0, 0), "radius": 0.1273},
}

def create_wheel_circle(name, x, y, z, radius, vertices):
    ...
    for i in range(num_points):
        ...
        bp = spline.bezier_points[i]
        bp.co = (x_local, y_local, 0)
        bp.handle_left_type = 'FREE'
        bp.handle_right_type = 'FREE'
        
        tangent_x = -math.sin(angle)
        tangent_y = math.cos(angle)
        handle_length = k * radius
        
        bp.handle_left = (
            x_local - handle_length * tangent_x,
            y_local - handle_length * tangent_y,
            0
        )
        bp.handle_right = (
            x_local + handle_length * tangent_x,
...
wheels = {}
for name, data in WHEELS_ORIGINAL.items():
    wheel = create_wheel_circle(
        name, 
        *data["pos"], 
        data["radius"], 
        data["vertices"] )
